                ...
                <div className="text-center mb-12">
                    ...
                    >
                        <button className="flex items-center gap-2 bg-white/10 hover:bg-white/20 border border-gold-500/50 text-gold-400 px-8 py-3 rounded-full font-medium transition-all backdrop-blur-sm group">
                            <Lock size={18} />
                            {t('agency_page.access_button')}
                            <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                        </button>
                    </motion.div>
                    <p className="text-xs text-gray-500 mt-3">{t('agency_page.disclaimer')}</p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
                    ...
                    >
                        <div className="prose prose-invert max-w-none">
                            <h3 className="text-2xl font-serif text-gold-500 mb-4">{t('agency_page.collaboration_model')}</h3>
                            <p className="text-gray-300 text-lg leading-relaxed">
                                {t('agency_page.collaboration_text_1').split('**').map((part, i) =>
                                    i % 2 === 1 ? <strong key={i} className="text-white">{part}</strong> : part
                                )}
                            </p>
                            <ul className="list-disc pl-5 mt-4 space-y-4 text-gray-400">
                                {(t('agency_page.benefits', { returnObjects: true }) || []).map((benefit, index) => (
                                    <li key={index}>
                                        <strong className="text-white block mb-1">{benefit.title}</strong>
                                        {benefit.desc}
                                    </li>
                                ))}
                            </ul>
                        ...
                        <div className="grid gap-6">
                            <div className="flex gap-4 p-4 bg-midnight-900/50 rounded-xl border border-white/5">
                                ...
                                <div>
                                    <h4 className="font-bold text-white">{t('agency_page.verified_demand_title')}</h4>
                                    <p className="text-gray-400 text-sm">{t('agency_page.verified_demand_text')}</p>
                                </div>
                            </div>
                            <div className="flex gap-4 p-4 bg-midnight-900/50 rounded-xl border border-white/5">
                                <Building2 className="text-gold-500 flex-shrink-0 mt-1" />
                                <div>
                                    <h4 className="font-bold text-white">{t('agency_page.global_exposure_title')}</h4>
                                    <p className="text-gray-400 text-sm">{t('agency_page.global_exposure_text')}</p>
                                </div>
                            </div>
                        </div>
                    ...
                    >
                        <div className="bg-midnight-800/80 backdrop-blur-md rounded-2xl border border-gold-500/20 p-1">
                            <div className="bg-midnight-950/50 rounded-xl p-6 md:p-8">
                                <div className="mb-6">
                                    <h3 className="text-xl font-bold text-white">{t('agency_page.form_title')}</h3>
                                    <p className="text-gray-400 text-sm mt-1">{t('agency_page.form_subtitle')}</p>
                                </div>
                                <AgencyContactForm explanation={t('agency_page.form_explanation')} />
                            </div>
                        </div>
                    </motion.div>
                </div>
