...
export default function MatchingEngine() {
    ...
    const calculateMatches = (props, leads) => {
        ...
        props.forEach(property => {
            const propertyMatches = leads.filter(lead => {
                ...
                // 2. Budget Match (Simple parsing)
                // TODO: Enhance budget parsing logic
                // For now, assume if the lead has ANY budget, it's a potential match if location matches
                // Refine this with actual budget string parsing later
    ...
    const parseBudget = (budgetString) => {
        // Placeholder for future robust parsing
        // E.g. "1M-5M" -> { min: 1000000, max: 5000000 }
        return null;
    };
        ...
        <div className="space-y-6">
            <div className="bg-gradient-to-r from-midnight-900 to-midnight-800 p-6 rounded-xl border border-gold-500/20">
                <h2 className="text-xl font-bold text-white mb-2 flex items-center gap-2">
                    <Users className="text-gold-500" /> Motor de Matching (Beta)
                </h2>
                <p className="text-gray-400 text-sm">
                    Este sistema cruza automáticamente tu inventario de propiedades con las demandas de los inversores basándose en ubicación y criterios clave.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6">
                {properties.map(property => {
                    ...
                    if (matchedLeads.length === 0) return null; // Optional: Hide properties with no matches?
                            ...
                            <div className="p-4 border-b border-white/5 bg-midnight-950/50 flex justify-between items-center">
                                <div className="flex items-center gap-4">
                                    <div className="h-12 w-12 bg-gray-800 rounded-lg flex items-center justify-center text-gray-500 text-xs">
                                        {property.images?.[0] ? <img src={property.images[0]} className="w-full h-full object-cover rounded-lg" /> : 'Sin Foto'}
                                    </div>
                                    <div>
                                ...
                                <div className="text-right">
                                    ...
                                    <p className="text-xs text-gray-500">Inversores Interesados</p>
                                </div>
                            </div>
                ...
                {properties.length > 0 && Object.values(matches).every(m => m.length === 0) && (
                    <div className="text-center py-12 text-gray-400">
                        No se encontraron coincidencias automáticas entre tus propiedades y los inversores actuales.
                    </div>
                )}
            </div>
        </div>
